...
class MultiFMacroApp:
    ...
    def _hover_tick(self):
        """Move mouse over the chat box centre every 20 seconds — no click."""
        if not self._hover_running:
            return
        # Auto-scan on first tick if not yet done
        if not getattr(self, "_ocr_chat_xy", None):
            xy = self._ocr_scan_chat_box()
            if xy:
                self._ocr_chat_xy = xy
                coord_var = getattr(self, "_ocr_coord_var", None)
                if coord_var:
                    self.root.after(0, lambda: coord_var.set(
                        f"x={xy[0]}  y={xy[1]}  ✓ auto-scanned"))
        if PAG_OK:
            ...
            if status:
                self.root.after(0, lambda: status.set(
                    f"Hovered chat at ({self._get_chat_xy()[0]},{self._get_chat_xy()[1]}) — next in 20s"))
