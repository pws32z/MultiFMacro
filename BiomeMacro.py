        ...
        def _loop():
            while self._chat_ocr_running:
                try:
                    ...
                    if text:
                        text_l = text.lower()
                        # Check each line individually so egg message isn't merged
                        # with unrelated chat lines into a false negative/positive
                        lines = [l.strip() for l in text_l.splitlines() if l.strip()]
                        # Match against known phrases from EGG_PHRASE_MAP
                        matched_egg = None
                        ...
                        for phrase, egg_name in EGG_PHRASE_MAP.items():
                            if phrase in full_text:
                                ...
                                break
                        # Fallback: any generic egg spawn signal
                        if matched_egg is None:
                            generic_triggers = ["egg spawned", "[egg", "egg has spawned"]
                            if any(t in full_text for t in generic_triggers):
                                matched_egg = "Special Egg"
                        print(f"[ChatOCR] tick | matched={matched_egg} | url={bool(url)} | {repr(text_l[:60])}")
