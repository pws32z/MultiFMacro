...
class MultiFMacroApp:
    ...
    def _build_merchant_tab(self):
        ...
        div(oc, 1, 4)
        # Tesseract status row
        tf2 = tk.Frame(oc, bg=BG3); tf2.grid(row=2, column=0, columnspan=4, sticky="ew", padx=12, pady=(6,0))
        ...
        # Delay settings row
        df = tk.Frame(oc, bg=BG2); df.grid(row=3, column=0, columnspan=4, sticky="w", padx=12, pady=(6,2))
        ...
        mf = tk.Frame(oc, bg=BG2); mf.grid(row=4, column=0, columnspan=4, sticky="w", padx=12, pady=(4,10))
        ...
        # ── Chat region calibration (shared with Egg tab) ─────────────────
        # Always ensure _ocr_coord_var and _ocr_scan_status exist before using them.
        # _ocr_chat_bbox is set in __init__ so we can't use it as a guard —
        # check _ocr_coord_var directly instead.
        _bx1_init, _by1_init, _bx2_init, _by2_init = self._ocr_chat_bbox
        if not hasattr(self, "_ocr_coord_var"):
            self._ocr_coord_var = tk.StringVar(
                value=f"x1={_bx1_init} y1={_by1_init}  x2={_bx2_init} y2={_by2_init}  ({_bx2_init-_bx1_init}×{_by2_init-_by1_init}px)")
        if not hasattr(self, "_ocr_scan_status"):
            self._ocr_scan_status = tk.StringVar(value="")

        mcf = tk.Frame(oc, bg=BG2); mcf.grid(row=5, column=0, columnspan=4, sticky="w", padx=12, pady=(8,2))
        ...
        msf = tk.Frame(oc, bg=BG2); msf.grid(row=6, column=0, columnspan=4, sticky="w", padx=12, pady=(2,8))
        ...
        def _open_merch_chat_calibrator():
            import io as _io
            sw = self.root.winfo_screenwidth()
            sh = self.root.winfo_screenheight()
            try:
            ...
            def _on_release(e):
                ...
                try:
                    ...
                    print(f"[MerchCalibrate] preview error: {_ex}")
            def _confirm(e=None):
                ...
                if coords and len(coords) == 4:
                    ...
                    try:
                        ...
                        print(f"[MerchCalibrate] save error: {ex}")
                else:
        ...
        btn(msf, "✏  Calibrate Chat Region", _open_merch_chat_calibrator, ACC).grid(row=0, column=0, padx=(0,6))
        btn(msf, "Reset Default",             _reset_merch_chat_bbox,      BG3).grid(row=0, column=1)
    ...
    def _build_fishing_tab(self):
        ...
        FS_ZIP_URL = f"https://github.com/pws32z/MultiFMacro/releases/download/{VERSION.upper()}/FishScope-Macro-2.4-Beta3.zip"
        MY_NAME = os.path.basename(CURRENT_FILE)

        def _is_downloaded():
            """Check if FishScope zip exists anywhere on common paths (Downloads, Desktop, etc)."""
            ...
            for d in search_dirs:
                if os.path.isfile(os.path.join(d, "FishScope-Macro-2.4-Beta3.zip")):
                    return True
            # Also check if folder exists anywhere near BASE_DIR
        ...
        tk.Frame(inner, bg=BDR, height=1).pack(fill="x", pady=14)
        tk.Label(inner, text="Auto Sell  \u00b7  Webhook stats  \u00b7  Auto Reconnect  \u00b7  Auto Calibration",
                 bg=BG1, fg=TXT2, font=("Segoe UI", 9), justify="center").pack(pady=(0, 14))
        ...
        self._fish_status_lbl.pack(pady=(0, 8))

        # Dynamic button area
        btn_frame = tk.Frame(inner, bg=BG1); btn_frame.pack()

        def _refresh_state():
            """Rebuild buttons based on current FishScope state."""
            for w in btn_frame.winfo_children():
                w.destroy()

            if os.path.isfile(FS_MAIN):
                # ── Ready to launch ──────────────────────────────────────
                self._fish_status_var.set("")
                self._fish_status_lbl.configure(fg=TXT2)
                btn(btn_frame, "  Launch FishScope  ", _launch, ACC).pack(pady=(0,8))
                bf = tk.Frame(btn_frame, bg=BG1); bf.pack(pady=(4,0))
                btn(bf, "Install Deps", _do_install_deps, BG3).pack(side="left", padx=(0,6))
                btn(bf, "GitHub", lambda: webbrowser.open("https://github.com/cresqnt-sys/FishScope-Macro"), BG3).pack(side="left", padx=(0,6))
                btn(bf, "Changelog", lambda: webbrowser.open("https://github.com/cresqnt-sys/FishScope-Macro/blob/main/CHANGELOG.md"), BG3).pack(side="left")

            elif _is_downloaded():
                # ── Downloaded but wrong folder ──────────────────────────
                self._fish_status_var.set(
                    f"FishScope is downloaded but not in the right place.\n"
                    f"Put  FishScope-Macro-2.4-Beta3  and  {MY_NAME}  in the same folder."
                )
                self._fish_status_lbl.configure(fg=GOLD)
                btn(btn_frame, "  Refresh  ", _refresh_state, BG3).pack(pady=(0,8))

            else:
                # ── Not downloaded yet ────────────────────────────────────
                self._fish_status_var.set("FishScope not downloaded yet.")
                self._fish_status_lbl.configure(fg=TXT2)
                btn(btn_frame, "  Download FishScope  ",
                    lambda: [webbrowser.open(FS_ZIP_URL),
                             self.root.after(1500, _refresh_state)],
                    ACC).pack(pady=(0,8))
                bf = tk.Frame(btn_frame, bg=BG1); bf.pack(pady=(4,0))
                btn(bf, "GitHub", lambda: webbrowser.open("https://github.com/cresqnt-sys/FishScope-Macro"), BG3).pack(side="left", padx=(0,6))
                btn(bf, "Refresh", _refresh_state, BG3).pack(side="left")

        def _find_python():
            import shutil as _sh
            next_to_exe = os.path.join(BASE_DIR, "python.exe")
                    ...
                    for base in (r"SOFTWARE\Python\PythonCore",
                        ...
                        try:
                            with winreg.OpenKey(root, base) as bk:
                                ...
                                while True:
                                    try:
                                        ver = winreg.EnumKey(bk, i)
                                        with winreg.OpenKey(bk, ver + r"\InstallPath") as ik:
                                            path = winreg.QueryValue(ik, None)
            ...
            home = os.path.expanduser("~")
            for c in [
                os.path.join(home, r"AppData\Local\Programs\Python\Python313\python.exe"),
                os.path.join(home, r"AppData\Local\Programs\Python\Python312\python.exe"),
                os.path.join(home, r"AppData\Local\Programs\Python\Python311\python.exe"),
                os.path.join(home, r"AppData\Local\Programs\Python\Python310\python.exe"),
        ...
        def _do_install_deps():
            """Install missing deps in background, then launch."""
            python = _find_python()
            if not python:
                self._fish_status_var.set("Python not found — install from python.org (tick Add to PATH)")
                self._fish_status_lbl.configure(fg=ACC3)
                return
            self._fish_status_lbl.configure(fg=ACC2)
            def _bg():
                def _cb(msg, col):
                    color = {"green": GREEN, "cyan": ACC2, "red": ACC3}.get(col, TXT2)
                    self.root.after(0, lambda: self._fish_status_var.set(msg))
                    self.root.after(0, lambda: self._fish_status_lbl.configure(fg=color))
                installed, failed = _install_deps(python, _cb)
                if failed:
                    self.root.after(0, lambda: self._fish_status_var.set(
                        f"✗ Failed: {', '.join(failed)} — check internet connection"))
                    self.root.after(0, lambda: self._fish_status_lbl.configure(fg=ACC3))
                else:
                    self.root.after(0, lambda: self._fish_status_var.set(
                        f"✓ All deps ready ({installed} installed)" if installed else "✓ Already up to date"))
                    self.root.after(0, lambda: self._fish_status_lbl.configure(fg=GREEN))
                    self.root.after(500, lambda: _refresh_state())
            threading.Thread(target=_bg, daemon=True).start()

        def _launch():
            if not os.path.isfile(FS_MAIN):
                _refresh_state(); return
            python = _find_python()
            if not python:
                self._fish_status_var.set("Python not found — install from python.org (tick Add to PATH)")
                self._fish_status_lbl.configure(fg=ACC3)
                return
            # Check deps first — install silently if missing
            missing = [pip for imp, pip in _DEPS.items() if not _check_dep(imp)]
            if missing:
                self._fish_status_var.set(f"Installing {len(missing)} missing package(s)…")
                self._fish_status_lbl.configure(fg=ACC2)
                def _bg():
                    _install_deps(python)
                    self.root.after(0, lambda: _do_launch(python))
                threading.Thread(target=_bg, daemon=True).start()
            else:
                _do_launch(python)

        def _do_launch(python):
            try:
        ...
        tk.Frame(inner, bg=BDR, height=1).pack(fill="x", pady=14)
        tk.Label(inner, text=f"Place FishScope-Macro-2.4-Beta3 folder next to  {MY_NAME}",
                 bg=BG1, fg=TXT3, font=("Segoe UI", 7), justify="center").pack()
    ...
    def _build_egg_tab(self):
        ...
        # ── Chat calibration ──────────────────────────────────────────────
        # Load saved bbox from config, fall back to 1080p default
        _saved_bbox = (
        ...
        cf = tk.Frame(hc, bg=BG2); cf.grid(row=3, column=0, columnspan=4, sticky="w", padx=12, pady=(8,2))
        tk.Label(cf, text="Chat region:", bg=BG2, fg=TXT3, font=FONT_LABEL).pack(side="left")
        tk.Label(cf, textvariable=self._ocr_coord_var, bg=BG2, fg=ACC2, font=FONT_MONO).pack(side="left", padx=(6,0))

        sf = tk.Frame(hc, bg=BG2); sf.grid(row=4, column=0, columnspan=4, sticky="w", padx=12, pady=(2,4))
        tk.Label(sf, textvariable=self._ocr_scan_status, bg=BG2, fg=GOLD, font=FONT_LABEL).grid(row=1, column=0, columnspan=4, sticky="w", pady=(2,0))

        def _open_calibrator():
            """Open a fullscreen transparent overlay where the user draws a box over their chat.
            On mouse release a screenshot of the drawn region is taken and shown as a preview."""
            import io as _io
            sw = self.root.winfo_screenwidth()
            sh = self.root.winfo_screenheight()

            # ── 1. Take a frozen screenshot of the whole screen first ─────────
            try:
                from PIL import ImageGrab as _IG, ImageTk as _ITK
                _bg_shot = _IG.grab(all_screens=True)
            except Exception:
            ...
            # Show the frozen screenshot as background so user can see their chat
            cv = tk.Canvas(ov, cursor="crosshair", highlightthickness=0, bd=0,
                           bg="black")
            cv.pack(fill="both", expand=True)
            ...
            if _bg_shot:
                try:
                    ...
                    _bg_tk = _ITK.PhotoImage(_bg_shot_resized, master=ov)
                    cv.create_image(0, 0, anchor="nw", image=_bg_tk)
                    cv._bg_tk = _bg_tk  # keep reference
                except Exception:
                    pass

            # Dark translucent overlay tinted on top
            cv.create_rectangle(0, 0, sw, sh, fill="black", stipple="gray50", outline="")
            ...
            def _on_press(e):
                ...
                if _state["rect"]: cv.delete(_state["rect"])
                cv.delete("sizelabel")
                _state["rect"] = cv.create_rectangle(
                    e.x, e.y, e.x, e.y,
                    outline="#7c6af7", width=3, fill="")
            ...
            def _on_release(e):
                """On mouse release: show screenshot preview of the drawn region."""
                if not _state["rect"]: return
                coords = cv.coords(_state["rect"])
                if not coords or len(coords) != 4: return
                rx1, ry1, rx2, ry2 = [int(v) for v in coords]
                ...
                # Take a fresh screenshot of just that region
                try:
                    from PIL import ImageGrab as _IG2, ImageTk as _ITK2
                    try:
                        snap = _IG2.grab(bbox=(bx1, by1, bx2, by2), all_screens=True)
                    ...
                    # Scale preview to max 500×350 while keeping aspect ratio
                    pw, ph = snap.size
                    max_w, max_h = 500, 350
                    scale = min(max_w / max(pw, 1), max_h / max(ph, 1), 1.0)
                    if scale < 1.0:
                    ...
                    tk.Label(prev, text="Screenshot of selected region:",
                             bg="#1a1a2e", fg="white",
                             font=("Segoe UI", 10, "bold")).pack(pady=(8, 4))
                    lbl = tk.Label(prev, image=snap_tk, bg="#1a1a2e",
                                   relief="solid", bd=1)
                    lbl.image = snap_tk  # keep reference
                    lbl.pack(padx=10)
        ...
        btn(sf, "✏  Calibrate Chat Region", _open_calibrator, ACC).grid(row=0, column=0, padx=(0,6))
        btn(sf, "Reset Default",             _reset_bbox,       BG3).grid(row=0, column=1)
        ...
        # Tesseract status banner (always visible)
        ...
        def _save_debug():
            ...
            def _do():
                try:
                    ...
                    if y1 <= y0:
                        y0, y1 = 55, 360  # reset to hardcoded fallback if scan broke it
    ...
    def _build_credits_tab(self):
        ...
        def _manual_check():
            ...
            def _do():
                result = check_for_update()
                if not result:
                    self.root.after(0, lambda: self._update_status_var.set(f"Up to date ({VERSION}) — check console for details"))
                    return
                tag, url, size = result
                # Build a small update window with progress bar
                def _start_update():
                    win = tk.Toplevel(self.root)
                    ...
                    win.attributes("-topmost", True)
                    win.grab_set()
                    tk.Label(win, text=f"Updating to {tag}", bg="#0a0a0a", fg="#c8c8d8",
                             font=("Segoe UI", 11, "bold")).pack(pady=(18, 4))
                    sv = tk.StringVar(value="Starting...")
                    tk.Label(win, textvariable=sv, bg="#0a0a0a", fg="#5ce1e6",
                             font=("Segoe UI", 9)).pack()
        ...
        btn(inner, "  Check for Updates  ", _manual_check, color=ACC).pack()
        tk.Label(inner, textvariable=self._update_status_var, bg=BG1, fg=ACC2,
                 font=FONT_LABEL).pack(pady=(6,0))
    ...
    def _ocr_read_chat_text(self, save_debug=False):
        """
        Screenshot the Roblox chat panel and return its text via pytesseract.
        ...
        try:
            # Use calibrated bbox (set by Calibrate Chat Region or loaded from config)
            x0, y0, x1, y1 = getattr(self, "_ocr_chat_bbox", (0, 50, 470, 400))
            print(f"[ChatOCR] using calibrated bbox: ({x0},{y0})→({x1},{y1})")
