...
try:
    ...
    BIOME_COLORS = {
        ...
        "GLITCHED":0x65FF00,"DREAMSPACE":0xFF7DFF,"EGGLAND":0xFFD700,
    ...
    EGGLAND_IMG     = "https://cresqnt.com/api/images/EGGLAND.png"
    ...
    CYBERSPACE_ALWAYS_ON = {"GLITCHED", "DREAMSPACE", "CYBERSPACE", "SINGULARITY"}
    ...
    # Mutable theme state — always reflects current live colours
    ...
    def _get_latest_log_content():
        ...
        try:
            ...
            # A real game session log has hundreds of lines; skip tiny files (< 50 KB)
            logs = [f for f in all_logs if os.path.getsize(f) > 50_000]
            if not logs:
                # Fallback: use any non-excluded log if none are large enough
                logs = all_logs
            if not logs:
                print(f"[LogReader] ERROR: No valid .log files found in {d}")
                return None
            # Pick the most recently MODIFIED (not created) — handles log rotation
    ...
    _log_dumped = [False]  # dump raw lines only once per session
    ...
    def get_latest_hovertext():
        ...
        # One-time raw dump so we can see the actual log format
        ...
        # Use a greedy pattern that matches from first { to last } on the line
    ...
    def make_egg_embed(egg_name, ps="", disc_id="", ping=False):
        ...
        # Use EGGLAND image as thumbnail for any eggland-related egg
        if egg_name and "eggland" in egg_name.lower():
            embed["thumbnail"] = {"url": EGGLAND_IMG}
        payload = {"embeds": [embed]}
        if ping and disc_id:
            payload["content"] = f"<@{disc_id}>"
        return payload
    ...
    def do_anti_kick():
        """
        Click the centre of the Roblox window to prevent idle kick.
        Saves the current mouse pos, briefly restores/foregrounds Roblox,
        clicks its centre, restores the mouse and previous foreground window.
        Re-minimises Roblox if it was minimised before.
        """
        import ctypes
    ...
    class MultiFMacroApp:
        ...
        def _build_gui(self):
            ...
            TABS = ["WEBHOOK","AURA","DETECTIONS","ANTIKICK","FISHING","LEADERBOARD","REMOTE","UI","CREDITS"]
        ...
        def _build_webhook_tab(self):
            ...
            # ══ Bottom: Biome Stats (full width, all sessions) ════════════════
        ...
        def _build_detections_tab(self):
            ...
            # EGG SECTION
            # ══════════════════════════════════════════════════════════════════
            tk.Frame(root, bg=GOLD, height=3).pack(fill="x", pady=(6,6))
            tk.Label(root, text="🥚  EGG", bg=BG1, fg=GOLD,
                     font=("Segoe UI", 11, "bold")).pack(anchor="w", padx=4, pady=(0,6))
            ...
            if not hasattr(self, "_keep_chat_var"):
                self._keep_chat_var = tk.BooleanVar(value=False)
            tk.Frame(of, bg=BG2, width=1, height=20).pack(side="left", padx=(14,0))
            def _toggle_keep_chat_egg():
                if self._keep_chat_var.get():
                    self._start_keep_chat()
                else:
                    self._stop_keep_chat()
            tk.Checkbutton(of, text="Keep Chat Open", variable=self._keep_chat_var,
                           command=_toggle_keep_chat_egg,
                           bg=BG2, fg=TXT2, selectcolor=BG3, activebackground=BG2,
                           font=FONT_LABEL, cursor="hand2").pack(side="left", padx=(6,0))
        ...
        def _apply_theme_live(self, bg0, bg1, acc):
            """
            Repaint every widget to new theme colours.
            Uses THEME dict so repeated calls always map from *current* → new.
            Skips any widgets registered in self._swatch_widgets (accent swatches).
            """
            old_bg0 = THEME["bg0"]
            old_bg1 = THEME["bg1"]
            old_bg2 = THEME["bg2"]
            ...
            # derive bg2/bg3 by lightening bg1 slightly
            ...
            # Widgets that must never be recoloured (accent swatches, etc.)
            protected = getattr(self, "_swatch_widgets", set())

            def _repaint(widget):
                if widget in protected:
                    return   # skip swatch — keep its fixed colour
                try:
                    cur = widget.cget("bg")
                    new = mapping.get(cur)
                    if new:
            ...
            # Update the global THEME so next call maps from new colours
            THEME["bg0"] = bg0
            THEME["bg1"] = bg1
            THEME["bg2"] = new_bg2
            THEME["bg3"] = new_bg3
            ...
            # Re-apply tab highlights so the active tab bar uses the new accent
        ...
        def _build_ui_tab(self):
            ...
            for i, (name, bg0, bg1, acc) in enumerate(PRESETS):
                ...
                # Protect all widgets inside the preset card from theme repainting
            ...
            for hex_col, cname in ACCENTS:
                ...
                # Protect this swatch from being recoloured by _apply_theme_live
                if not hasattr(self, "_swatch_widgets"):
                    self._swatch_widgets = set()
        ...
        def _start(self):
            ...
            self.last_aura  = ""  # Don't pre-seed — fire webhook for currently equipped aura on first poll
        ...
        def _check(self):
            ...
            if biome is None:
                pass  # log unreadable — keep last known biome, don't reset state
        ...
        def _keep_chat_tick(self):
            """
            Every 8s: click the calibrated chat region to keep it open for OCR.
            ...
            def _do_click():
                try:
                    if not self._keep_chat_open:
                        return
                    # Use the calibrated chat centre if available
                    xy = getattr(self, "_ocr_chat_xy", None)
                    if not xy:
        ...
        def _grab_region(x0, y0, x1, y1):
            """
            Grab a screen region — works even when Roblox is behind other windows
            or minimized, using PrintWindow with PW_RENDERFULLCONTENT (works on
            DirectX/GPU rendered windows like Roblox).
            Falls back to mss / pyautogui / ImageGrab if PrintWindow fails.
            ...
            # ── PrintWindow PW_RENDERFULLCONTENT (works on DirectX / Roblox) ──
            try:
        ...
        def _ocr_read_chat_text(self, save_debug=False, bbox=None):
            ...
            try:
                ...
                # ── Scale up 4x for better OCR accuracy ──────────────────────
                shot = shot.resize((w * 4, h * 4), Image.LANCZOS)
                arr = _np.array(shot.convert("RGB")).astype(_np.int32)

                # ── Multi-threshold approach: try several thresholds and pick
                #    the one that produces the most text ─────────────────────
                best_text = ""
                # Roblox chat text can range from ~20 brightness (dim) to 200+ (white)
                ...
                # ── Also try a contrast-enhanced greyscale version ────────────
                try:
                    grey = shot.convert("L")
