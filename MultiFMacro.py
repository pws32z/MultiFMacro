...
try:
    ...
    BIOME_COLORS = {
        ...
        "HEAVEN":0xFFFFFF,"SINGULARITY":0xFF4500,
    }

    BASE_IMG = "https://raw.githubusercontent.com/bazthedev/SolsScope/refs/heads/main/img/biome/"
    HEAVEN_IMG      = "https://cresqnt.com/api/images/HEAVEN.png"
    SINGULARITY_IMG = "https://github.com/pws32z/MultiFMacro/blob/main/assets/SINGULARITY.png?raw=true"
    BIOME_IMAGES = {
    ...
    THEME = {"bg0": "#0a0a0a", "bg1": "#0f0f0f", "bg2": "#141414", "bg3": "#1a1a1a", "acc": "#7c6af7"}
    ...
    def _get_latest_log_content():
        d = _get_log_dir()
        if not d:
            print("[LogReader] ERROR: Could not find Roblox log directory!")
            return None
        try:
            EXCLUDE = ("Installer", "bootstrapper", "crash", "Crash")
            all_logs = [f for f in glob.glob(os.path.join(d,"*.log"))
                        if not any(x in f for x in EXCLUDE)]
            logs = [f for f in all_logs if os.path.getsize(f) > 50_000]
            if not logs:
                logs = all_logs
            if not logs:
                print(f"[LogReader] ERROR: No valid .log files found in {d}")
                return None
            ...
            print(f"[LogReader] OK: {os.path.basename(latest)} — {len(content)} lines ({os.path.getsize(latest)//1024} KB)")
            return content
        except Exception as e:
    ...
    def get_latest_hovertext():
        ...
        if not _log_dumped[0]:
            _log_dumped[0] = True
            print("=" * 60)
            print(f"[LogDump] Showing last 30 lines of log ({len(content)} total):")
            for i, ln in enumerate(content[-30:]):
                print(f"  [{len(content)-30+i}] {ln.rstrip()}")
            print("=" * 60)
        ...
        for line in reversed(content):
            ...
            try:
                ...
                if ht:
                    print(f"[Biome] Found hoverText={ht!r} after checking {checked} JSON lines")
                    return ht.strip().upper()
            except (json.JSONDecodeError, Exception):
                continue
        print(f"[Biome] No hoverText found after checking {checked} JSON lines")
        return None
    ...
    def get_latest_equipped_aura():
        ...
        for line in reversed(content):
            ...
            try:
                ...
                state = state.strip()
                print(f"[Aura] Found state={state!r} after checking {checked} JSON lines")
                ...
                continue
        print(f"[Aura] No equipped aura state found after checking {checked} JSON lines")
        return None
    ...
    def make_biome_embed(biome, is_start, ps, disc_id, setting, ping_biomes=None):
        ...
        ps_ln = f"\n[🔗 Join Private Server]({ps})" if (ps and is_start) else ""
        embed = {
            "title": f"[{ts}]",
            "color": color,
        ...
        if is_start:
            if biome in CYBERSPACE_ALWAYS_ON:
                # always-on biomes: @everyone + user mention if set
    ...
    def do_anti_kick():
        ...
        try:
            ...
            if not hwnd:
                print("[AntiKick] Roblox window not found — is Roblox open?")
                return

            SW_RESTORE  = 9
            SW_MINIMIZE = 6
            was_minimised = bool(user32.IsIconic(hwnd))
            ...
            print(f"[AntiKick] Clicked Roblox centre ({cx},{cy}), hwnd={hwnd}")
            return

        except Exception as e:
            print(f"[AntiKick] Click failed: {e}")
    ...
    SB_URL        = "https://xdbhgjepjrkyhcbdtdno.supabase.co/rest/v1"
    SB_ANON_KEY   = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhkYmhnamVwanJreWhjYmR0ZG5vIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODAxMzczNDAsImV4cCI6MjA5NTcxMzM0MH0.aLt6V-a2V5lzfwK0xs6dKbM5QUm6rzk2hOxpkpQrDPE"
    SB_FN_URL     = "https://xdbhgjepjrkyhcbdtdno.supabase.co/functions/v1/rapid-worker"
    SB_READ_HDR   = {"apikey": SB_ANON_KEY, "Authorization": f"Bearer {SB_ANON_KEY}"}

    def lb_fetch():
        try:
            r = requests.get(f"{SB_URL}/leaderboard?select=username,seconds&order=seconds.desc",
                             headers=SB_READ_HDR, timeout=8)
            print(f"[LB] fetch status={r.status_code}")
            if r.status_code == 200:
                rows = r.json()
                return {row["username"]: row["seconds"] for row in rows}
        except Exception as e:
            print(f"[LB] fetch error: {e}")
        return {}

    def lb_update(username, seconds_to_add):
        if not username or seconds_to_add < 5:
            print(f"[LB] skipped: username={username!r} seconds={seconds_to_add}")
            return
        try:
            seconds_to_add = min(int(seconds_to_add), 3600)
            resp = requests.post(SB_FN_URL,
                                 headers={"Content-Type": "application/json",
                                          "Authorization": f"Bearer {SB_ANON_KEY}"},
                                 json={"username": username, "seconds_to_add": seconds_to_add},
                                 timeout=10)
            print(f"[LB] update {username} += {seconds_to_add}s  status={resp.status_code} body={resp.text[:100]}")
        except Exception as e:
    ...
    class MultiFMacroApp:
        def __init__(self, root):
            ...
            self.last_eden_ts = time.time()
            self._last_eden_sent_at = 0.0
            self._last_biome_sent_at = {}
            self._merchant_history = []
            self._eden_history     = []
            self.biome_counts = {}
            ...
            self._ocr_watch_running = False
            self._ocr_watch_result  = False
            self._hover_running = False
            self._hover_job = None
            self._ocr_chat_xy = None
            self._chat_ocr_running = False
            self._eden_ocr_running   = False
            self._keep_chat_open     = False
            self._keep_chat_job      = None
        ...
        def _build_gui(self):
            ...
            TABS = ["WEBHOOK","AURA","DETECTIONS","ANTIKICK","FISHING","AUTOCRAFT","LEADERBOARD","REMOTE","UI","CREDITS"]
        ...
        def _show_tab(self, name):
            ...
            for n,(f,b,bar) in self._tab_btns.items():
                active = n == name
                _bg0 = THEME["bg0"]; _bg1 = THEME["bg1"]; _acc = THEME["acc"]
                f.configure(bg=_bg1 if active else _bg0)
                b.configure(bg=_bg1 if active else _bg0,
                            fg=TXT if active else TXT2,
                            font=("Segoe UI",8,"bold") if active else ("Segoe UI",8))
                bar.configure(bg=_acc if active else _bg0)
        ...
        def _build_webhook_tab(self):
            ...
            outer.rowconfigure(0, weight=0); outer.rowconfigure(2, weight=1)

            left = tk.Frame(outer, bg=BG1, padx=16, pady=14)
            ...
            for r, (lb, var) in enumerate(fields):
                tk.Label(c, text=lb, bg=BG2, fg=TXT2, font=FONT_LABEL, anchor="w").grid(
                    row=r+2, column=0, sticky="w", padx=(10,5), pady=4)
                entry(c, var, width=30).grid(
                    row=r+2, column=1, sticky="ew", padx=(0,10), pady=4)
            c.columnconfigure(1, weight=1)

            apply_status = tk.StringVar(value="")
            bf = tk.Frame(c, bg=BG2); bf.grid(row=6, column=0, columnspan=2, sticky="w", padx=10, pady=(2,8))
            def _apply_username():
            ...
            tk.Label(bf, textvariable=apply_status, bg=BG2, fg=GREEN, font=FONT_LABEL).pack(side="left", padx=(8,0))

            tk.Frame(outer, bg=BDR, width=1).grid(row=0, column=0, sticky="nse", pady=14)
            right = tk.Frame(outer, bg=BG1, padx=16, pady=14)
            right.grid(row=0, column=1, sticky="nsew")

            page_header(right, "Biome Ping", "notify when a biome starts")

            ao = tk.Frame(right, bg=BG1); ao.pack(anchor="w", pady=(0,4))
            tk.Label(ao, text="@everyone always:", bg=BG1, fg=TXT2, font=FONT_LABEL).pack(side="left")
            ...
            tk.Frame(right, bg=BDR, height=1).pack(fill="x", pady=(0,6))

            self._ping_biome_vars = {}
            PINGABLE = sorted(b for b in BIOME_COLORS if b not in CYBERSPACE_ALWAYS_ON)
            ping_grid = tk.Frame(right, bg=BG1); ping_grid.pack(anchor="w", fill="x")
            for i, biome in enumerate(PINGABLE):
                ...
                row_f = tk.Frame(ping_grid, bg=BG1)
                row_f.grid(row=i//2, column=i%2, sticky="w", padx=(0,16), pady=1)
            ...
            tk.Frame(outer, bg=BDR, height=1).grid(row=1, column=0, columnspan=2, sticky="ew", padx=16)

            stats_outer = tk.Frame(outer, bg=BG1, padx=16, pady=10)
            stats_outer.grid(row=2, column=0, columnspan=2, sticky="nsew")

            shdr = tk.Frame(stats_outer, bg=BG1); shdr.pack(fill="x", pady=(0,8))
            tk.Label(shdr, text="Biome Stats", bg=BG1, fg=TXT,
                     font=("Segoe UI",10,"bold")).pack(side="left")
            tk.Label(shdr, text="  all sessions", bg=BG1, fg=TXT3, font=FONT_LABEL).pack(side="left", pady=2)
            btn(shdr, "Reset All", self._reset_biome_stats, BG3).pack(side="right")

            self._stats_frame = tk.Frame(stats_outer, bg=BG1)
        ...
        def _refresh_biome_stats(self):
            ...
            for i, (biome, count) in enumerate(sorted_biomes):
                ...
                chip = tk.Frame(self._stats_frame, bg=BG2,
                                highlightthickness=1, highlightbackground=BDR)
                chip.grid(row=i//cols, column=i%cols, sticky="w", padx=(0,6), pady=3, ipadx=8, ipady=6)
                tk.Label(chip, text="●", bg=BG2, fg=color, font=("Segoe UI",9,"bold")).pack(side="left", padx=(6,3))
                tk.Label(chip, text=biome.title(), bg=BG2, fg=TXT2, font=("Segoe UI",8)).pack(side="left")
        ...
        def _reset_biome_stats(self):
            ...
            if self.cfg.has_section("BiomeCounts"):
                self.cfg.remove_section("BiomeCounts")
            self._save_config()
        ...
        def _build_detections_tab(self):
            ...
            page_header(root, "Detections", "merchant & eden detection")

            tk.Frame(root, bg="#7c6af7", height=3).pack(fill="x", pady=(0,6))
            tk.Label(root, text="🛒  MERCHANT", bg=BG1, fg="#7c6af7",
                     font=("Segoe UI", 11, "bold")).pack(anchor="w", padx=4, pady=(0,6))
            ...
            if not hasattr(self, "_keep_chat_var"):
                self._keep_chat_var = tk.BooleanVar(value=False)
            tk.Frame(mf, bg=BG2, width=1, height=20).pack(side="left", padx=(14,0))
            def _toggle_keep_chat_merch():
                if self._keep_chat_var.get():
                    self._start_keep_chat()
                else:
                    self._stop_keep_chat()
            tk.Checkbutton(mf, text="Keep Chat Open", variable=self._keep_chat_var,
                           command=_toggle_keep_chat_merch,
                           bg=BG2, fg=TXT2, selectcolor=BG3, activebackground=BG2,
                           font=FONT_LABEL, cursor="hand2").pack(side="left", padx=(6,0))
            ...
            tk.Frame(root, bg="#9900ff", height=3).pack(fill="x", pady=(6,6))
            tk.Label(root, text="🌌  EDEN", bg=BG1, fg="#9900ff",
                     font=("Segoe UI", 11, "bold")).pack(anchor="w", padx=4, pady=(0,6))
            ...
            if not hasattr(self, "_keep_chat_var"):
                self._keep_chat_var = tk.BooleanVar(value=False)
            tk.Frame(emf, bg=BG2, width=1, height=20).pack(side="left", padx=(14,0))
            def _toggle_keep_chat_eden():
                if self._keep_chat_var.get():
                    self._start_keep_chat()
                else:
                    self._stop_keep_chat()
            tk.Checkbutton(emf, text="Keep Chat Open", variable=self._keep_chat_var,
                           command=_toggle_keep_chat_eden,
                           bg=BG2, fg=TXT2, selectcolor=BG3, activebackground=BG2,
                           font=FONT_LABEL, cursor="hand2").pack(side="left", padx=(6,0))
        ...
        def _build_antikick_tab(self):
            ...
            page_header(root, "Anti-Kick", "clicks the Roblox window on a timer to prevent idle kick")
            ...
            def _manual_antikick():
                def _press():
                    do_anti_kick()
                    try:
                        self.root.after(0, lambda: self.ak_status_var.set("✓ Clicked Roblox — done manually"))
                    except RuntimeError:
                        pass
                threading.Thread(target=_press, daemon=True).start()

            btn(sc, "Click Now", _manual_antikick).grid(
                row=2, column=2, sticky="e", padx=(6,12), pady=(8,8))
            ...
            notes = [
                ...
                "• It clicks the centre of the Roblox window to reset the idle timer.",
                "• Roblox is briefly brought to the front (~300 ms), then your window is restored.",
        ...
        def _build_autocraft_tab(self):
            root = tk.Frame(self.content_area, bg=BG1); self._reg("AUTOCRAFT", root)
            root.configure(padx=18, pady=14)

            page_header(root, "Auto Craft",
                        "calibrate 4 click spots + OCR region, then loop crafting automatically")

            # ── calibration state ──────────────────────────────────────────
            CAL_KEYS = [
                ("add_all_1",   "① Add All (first time)"),
                ("tick_warn",   "② Tick Warning checkbox"),
                ("add_all_2",   "③ Add All (after tick)"),
                ("craft_btn",   "④ Craft button"),
            ]
            OCR_KEYS = [
                ("ocr_x1", "OCR X1"), ("ocr_y1", "OCR Y1"),
                ("ocr_x2", "OCR X2"), ("ocr_y2", "OCR Y2"),
            ]

            # load saved calibration
            def _load_cal(key, fallback=0):
                return self.cfg.getint("AutoCraft", key, fallback=fallback)

            self._ac_cal   = {k: [_load_cal(k+"_x"), _load_cal(k+"_y")] for k,_ in CAL_KEYS}
            self._ac_ocr   = {k: tk.IntVar(value=_load_cal(k, fb))
                              for k, fb in [("ocr_x1",0),("ocr_y1",0),
                                            ("ocr_x2",1920),("ocr_y2",1080)]}
            self._ac_loops     = tk.IntVar(value=self.cfg.getint("AutoCraft","loops",fallback=10))
            self._ac_delay     = tk.DoubleVar(value=self.cfg.getfloat("AutoCraft","delay",fallback=1.2))
            self._ac_skip_tick = tk.BooleanVar(value=self.cfg.getboolean("AutoCraft","skip_tick",fallback=False))
            self._ac_status    = tk.StringVar(value="Idle")
            self._ac_loop_var  = tk.StringVar(value="—")
            self._ac_cal_btns  = {}

            def _save_ac_config():
                if not self.cfg.has_section("AutoCraft"):
                    self.cfg["AutoCraft"] = {}
                for k,_ in CAL_KEYS:
                    self.cfg["AutoCraft"][k+"_x"] = str(self._ac_cal[k][0])
                    self.cfg["AutoCraft"][k+"_y"] = str(self._ac_cal[k][1])
                for k in self._ac_ocr:
                    self.cfg["AutoCraft"][k] = str(self._ac_ocr[k].get())
                self.cfg["AutoCraft"]["loops"] = str(self._ac_loops.get())
                self.cfg["AutoCraft"]["delay"] = str(self._ac_delay.get())
                self.cfg["AutoCraft"]["skip_tick"] = str(int(self._ac_skip_tick.get()))
                save_config(self.cfg)

            def _refresh_cal_labels():
                for k, _ in CAL_KEYS:
                    x, y = self._ac_cal[k]
                    txt = f"({x}, {y})" if (x or y) else "not set"
                    col = GREEN if (x or y) else TXT3
                    lbl_w = self._ac_cal_btns.get(k+"_lbl")
                    if lbl_w:
                        lbl_w.configure(text=txt, fg=col)

            # ── Calibration card ───────────────────────────────────────────
            cc = card(root); cc.pack(fill="x", pady=(0,8))
            sec(cc, "CLICK CALIBRATION", 0, 3); div(cc, 1, 3)
            cc.columnconfigure(1, weight=1)

            cal_info = tk.Label(cc, text="Move mouse to target, press Calibrate — coords captured from cursor position.",
                                bg=BG2, fg=TXT3, font=FONT_LABEL, anchor="w", justify="left")
            cal_info.grid(row=2, column=0, columnspan=3, sticky="w", padx=12, pady=(6,4))

            for r, (key, label) in enumerate(CAL_KEYS):
                row_r = r + 3

                tk.Label(cc, text=label, bg=BG2, fg=TXT2, font=FONT_LABEL,
                         anchor="w", width=26).grid(row=row_r, column=0, sticky="w",
                                                    padx=(12,4), pady=3)

                coord_lbl = tk.Label(cc, text="not set", bg=BG2, fg=TXT3,
                                     font=FONT_MONO, anchor="w", width=14)
                coord_lbl.grid(row=row_r, column=1, sticky="w", padx=4)
                self._ac_cal_btns[key+"_lbl"] = coord_lbl

                _key = key
                def _make_cal_fn(k, wait_secs=3):
                    def _do_cal():
                        self._ac_status.set(f"Move mouse to {k!r} — capturing in {wait_secs}s…")
                        def _capture():
                            ...
                            try:
                                ...
                                self._ac_cal[k][0] = pt.x
                                self._ac_cal[k][1] = pt.y
                            except Exception:
                                ...
                                self._ac_cal[k][1] = my
                            try:
                                self.root.after(0, _refresh_cal_labels)
                                self.root.after(0, lambda: self._ac_status.set(
                                    f"✓ Calibrated {k}: ({self._ac_cal[k][0]}, {self._ac_cal[k][1]})"))
                                _save_ac_config()
                            except RuntimeError:
                                pass
                ...
                btn(cc, "Calibrate", _make_cal_fn(_key, wait_secs=7 if _key == "add_all_2" else 3), BG3).grid(
                    row=row_r, column=2, sticky="e", padx=(4,12), pady=3)

            _refresh_cal_labels()

            # ── OCR region card ────────────────────────────────────────────
            oc = card(root); oc.pack(fill="x", pady=(0,8))
            sec(oc, "OCR FAILSAFE REGION  (drag to select area that shows \"not enough\")", 0, 5)
            div(oc, 1, 5)
            oc.columnconfigure(4, weight=1)

            ocr_row = tk.Frame(oc, bg=BG2)
            ocr_row.grid(row=2, column=0, columnspan=5, sticky="w", padx=12, pady=(6,4))

            for i, (k, lbl_txt) in enumerate(OCR_KEYS):
                tk.Label(ocr_row, text=lbl_txt+":", bg=BG2, fg=TXT2,
                         font=FONT_LABEL).grid(row=0, column=i*2, sticky="w",
                                               padx=(0 if i==0 else 8, 2))
                e = tk.Entry(ocr_row, textvariable=self._ac_ocr[k],
                             bg=BG3, fg=TXT, insertbackground=ACC2, font=FONT_MONO,
                             relief="flat", highlightthickness=1, highlightbackground=BDR,
                             highlightcolor=ACC, bd=0, width=6)
                e.grid(row=0, column=i*2+1, padx=(0,4))

            self._ac_ocr_lbl = tk.Label(oc, text="No region set", bg=BG2, fg=TXT3,
                                        font=FONT_LABEL)
            self._ac_ocr_lbl.grid(row=3, column=0, columnspan=5, sticky="w",
                                  padx=12, pady=(0,4))
            # update label if already calibrated
            _ox1 = self._ac_ocr["ocr_x1"].get()
            _oy1 = self._ac_ocr["ocr_y1"].get()
            _ox2 = self._ac_ocr["ocr_x2"].get()
            _oy2 = self._ac_ocr["ocr_y2"].get()
            if _ox2 > _ox1 and _oy2 > _oy1:
                self._ac_ocr_lbl.configure(
                    text=f"Region: ({_ox1},{_oy1}) → ({_ox2},{_oy2})", fg=GREEN)

            def _cal_ocr_screenshot():
                """Take a full screenshot then open a fullscreen overlay for drag-selection."""
                ...
                def _run():
                    try:
                        ...
                        try:
                            self.root.after(0, lambda: self._ac_status.set(
                                f"Missing dep: {_ie} — pip install mss Pillow"))
                        except RuntimeError:
                            pass
                        return

                    # grab full virtual screen (all monitors)
                    try:
                        with mss.mss() as sct:
                            mon = sct.monitors[0]   # index 0 = all monitors combined
                            shot = sct.grab(mon)
                        ...
                        try:
                            self.root.after(0, lambda: self._ac_status.set(f"Screenshot failed: {_e}"))
                        except RuntimeError:
                            pass
                        return
                    ...
                    def _open_overlay():
                        ...
                        ov.overrideredirect(True)
                        ov.attributes("-topmost", True)
                        ov.geometry(f"{sw}x{sh}+{off_x}+{off_y}")
                        ov.configure(bg="black")

                        # show screenshot as canvas background
                        photo = ImageTk.PhotoImage(screen_img, master=ov)
                        cv = tk.Canvas(ov, width=sw, height=sh,
                        ...
                        hint = cv.create_text(sw // 2, 30,
                                              text="Drag to select the OCR region   |   ESC to cancel",
                        ...
                        def _press(e):
                            ...
                            rect_id[0] = cv.create_rectangle(
                                e.x, e.y, e.x, e.y,
                                outline="#5ce1e6", width=2, fill="")

                        def _drag(e):
                            if rect_id[0]:
                                cv.coords(rect_id[0], sx[0], sy[0], e.x, e.y)

                        def _release(e):
                            x1c = min(sx[0], e.x) + off_x
                            y1c = min(sy[0], e.y) + off_y
                            x2c = max(sx[0], e.x) + off_x
                            y2c = max(sy[0], e.y) + off_y
                            ov.destroy()
                            if abs(x2c - x1c) < 5 or abs(y2c - y1c) < 5:
                                try:
                                    self.root.after(0, lambda: self._ac_status.set(
                                        "Region too small — try again"))
                                except RuntimeError:
                                    pass
                                return
                            try:
                                self.root.after(0, lambda: self._ac_ocr["ocr_x1"].set(x1c))
                                self.root.after(0, lambda: self._ac_ocr["ocr_y1"].set(y1c))
                                self.root.after(0, lambda: self._ac_ocr["ocr_x2"].set(x2c))
                                self.root.after(0, lambda: self._ac_ocr["ocr_y2"].set(y2c))
                                self.root.after(0, lambda: self._ac_ocr_lbl.configure(
                                    text=f"Region: ({x1c},{y1c}) → ({x2c},{y2c})",
                                    fg=GREEN))
                                self.root.after(0, lambda: self._ac_status.set(
                                    f"✓ OCR region set: ({x1c},{y1c}) → ({x2c},{y2c})"))
                                _save_ac_config()
                            except RuntimeError:
                                pass
                    ...
                    try:
                        self.root.after(0, _open_overlay)
                    except RuntimeError:
                        pass

                _thr.Thread(target=_run, daemon=True).start()

            btn(oc, "📷  Select Region (drag on screenshot)", _cal_ocr_screenshot, ACC2).grid(
                row=4, column=0, columnspan=5, sticky="w", padx=12, pady=(0,8))

            # ── Settings card ──────────────────────────────────────────────
            sc = card(root); sc.pack(fill="x", pady=(0,8))
            sec(sc, "SETTINGS", 0, 4); div(sc, 1, 4)

            sf = tk.Frame(sc, bg=BG2); sf.grid(row=2, column=0, columnspan=4,
                                                sticky="w", padx=12, pady=(6,10))

            tk.Label(sf, text="Loops:", bg=BG2, fg=TXT2, font=FONT_LABEL).pack(side="left")
            tk.Entry(sf, textvariable=self._ac_loops, bg=BG3, fg=TXT,
                     insertbackground=ACC2, font=FONT_MONO, relief="flat",
                     highlightthickness=1, highlightbackground=BDR,
                     highlightcolor=ACC, bd=0, width=5).pack(side="left", padx=(4,16))

            tk.Label(sf, text="Delay (s):", bg=BG2, fg=TXT2, font=FONT_LABEL).pack(side="left")
            tk.Entry(sf, textvariable=self._ac_delay, bg=BG3, fg=TXT,
                     insertbackground=ACC2, font=FONT_MONO, relief="flat",
                     highlightthickness=1, highlightbackground=BDR,
                     highlightcolor=ACC, bd=0, width=5).pack(side="left", padx=(4,16))

            tk.Label(sf, text="between clicks", bg=BG2, fg=TXT3, font=FONT_LABEL).pack(side="left")

            # skip tick/add-all-2 checkbox
            skip_f = tk.Frame(sc, bg=BG2)
            skip_f.grid(row=3, column=0, columnspan=4, sticky="w", padx=12, pady=(0,10))
            skip_cb = tk.Checkbutton(
                skip_f,
                text="Skip steps ② Tick Warning + ③ Add All on every loop  (use if warning already ticked)",
                variable=self._ac_skip_tick,
                bg=BG2, fg=TXT2, selectcolor=BG3,
                activebackground=BG2, activeforeground=TXT,
                font=FONT_LABEL, anchor="w",
                command=_save_ac_config,
            )
            skip_cb.pack(side="left")

            # ── Status / control card ──────────────────────────────────────
            stc = card(root); stc.pack(fill="x", pady=(0,8))
            sec(stc, "CONTROL", 0, 3); div(stc, 1, 3)

            ctrl_f = tk.Frame(stc, bg=BG2)
            ctrl_f.grid(row=2, column=0, columnspan=3, sticky="ew", padx=12, pady=(8,10))

            self._ac_start_btn = btn(ctrl_f, "▶  Start Auto Craft", self._craft_start, ACC)
            self._ac_start_btn.pack(side="left")

            btn(ctrl_f, "■  Stop", self._craft_stop, "#bb3333").pack(side="left", padx=(6,0))

            tk.Label(ctrl_f, text="Loop:", bg=BG2, fg=TXT2,
                     font=FONT_LABEL).pack(side="left", padx=(20,4))
            tk.Label(ctrl_f, textvariable=self._ac_loop_var, bg=BG2, fg=ACC2,
                     font=("Segoe UI",9,"bold")).pack(side="left")

            stat_f = tk.Frame(stc, bg=BG2)
            stat_f.grid(row=3, column=0, columnspan=3, sticky="w", padx=12, pady=(0,10))
            tk.Label(stat_f, text="Status:", bg=BG2, fg=TXT2, font=FONT_LABEL).pack(side="left")
            tk.Label(stat_f, textvariable=self._ac_status, bg=BG2, fg=ACC2,
                     font=FONT_LABEL).pack(side="left", padx=(6,0))

            # ── How it works card ──────────────────────────────────────────
            hc = card(root); hc.pack(fill="x")
            sec(hc, "HOW IT WORKS", 0, 1); div(hc, 1, 1)
            info_f = tk.Frame(hc, bg=BG2); info_f.grid(row=2, column=0, sticky="ew", padx=12, pady=(6,10))
            notes = [
                "① Click calibrate for each of the 4 spots — you have 3 seconds to position your mouse.",
                "② On loop 1 (checkbox unchecked): clicks Add All → Tick Warning → Add All → Craft.",
                "   On loop 2+: skips steps ② and ③ (Tick Warning already ticked).",
                "   ☑ Skip checkbox: skips Tick Warning + Add All 2 on ALL loops (inc. loop 1).",
                "③ After each Craft click, OCR scans the calibrated region.",
                "   If 'not enough' is detected, crafting stops immediately.",
                "④ Calibrate the OCR region over the error message area in-game.",
                "⑤ Mouse movement is smoothed — no instant teleport.",
            ]
            for note in notes:
                tk.Label(info_f, text=note, bg=BG2, fg=TXT2, font=FONT_LABEL,
                         anchor="w", justify="left").pack(anchor="w", padx=8, pady=1)

        def _craft_start(self):
            if self._craft_running:
                return
            # validate click calibration
            missing = []
            for k, label in [("add_all_1","① Add All"),("tick_warn","② Tick Warning"),
                              ("add_all_2","③ Add All 2"),("craft_btn","④ Craft")]:
                x, y = self._ac_cal[k]
                if not (x or y):
                    missing.append(label)
            if missing:
                messagebox.showerror("Not Calibrated",
                    "Please calibrate these spots first:\n" + "\n".join(missing))
                return
            # snapshot ALL tk vars on main thread right now
            self._craft_snap = {
                "wh_url":     self.wh_url.get().strip(),
                "disc_id":    self.disc_id.get().strip(),
                "loops":      max(1, self._ac_loops.get()),
                "delay":      max(0.2, self._ac_delay.get()),
                "skip_tick":  self._ac_skip_tick.get(),
                "ocr_x1":     self._ac_ocr["ocr_x1"].get(),
                "ocr_y1":     self._ac_ocr["ocr_y1"].get(),
                "ocr_x2":     self._ac_ocr["ocr_x2"].get(),
                "ocr_y2":     self._ac_ocr["ocr_y2"].get(),
            }
            self._craft_running    = True
            self._ocr_watch_result = False
            try:
                self._ac_start_btn.configure(bg="#162e1e")
            except Exception:
                pass
            # start OCR watcher if region is calibrated
            s = self._craft_snap
            if s["ocr_x2"] > s["ocr_x1"] and s["ocr_y2"] > s["ocr_y1"]:
                self._ocr_watch_running = True
                threading.Thread(target=self._ocr_watcher, daemon=True).start()
            else:
                self._ocr_watch_running = False
            # start craft loop
            self._craft_thread = threading.Thread(target=self._craft_loop, daemon=True)
            self._craft_thread.start()

        def _craft_stop(self):
            self._craft_running     = False
            self._ocr_watch_running = False
            try:
                self._ac_start_btn.configure(bg=ACC)
            except Exception:
                pass
            try:
                self._ac_status.set("Stopped")
                self._ac_loop_var.set("—")
            except RuntimeError:
                pass

        def _ocr_watcher(self):
            """Continuous background scan — sets _ocr_watch_result=True when 'not enough' found."""
            try:
                import mss
                import pytesseract
                from PIL import Image, ImageOps, ImageFilter, ImageEnhance
            except Exception as e:
                print(f"[OCR watcher] missing dep: {e}")
                self._ocr_watch_running = False
                return

            # auto-locate tesseract if not already set
            if not getattr(pytesseract, "tesseract_cmd", None) or \
            ...
            # keywords that mean "not enough ingredients" — covers partial OCR misreads
            _BAD_PHRASES = [
                "not enough", "not enoug", "not enuf", "notenough",
                "nough ingr", "enough ingr", "insufficient", "not have enough",
            ]

            s  = self._craft_snap
            x1 = s["ocr_x1"]; y1 = s["ocr_y1"]
            x2 = s["ocr_x2"]; y2 = s["ocr_y2"]
            print(f"[OCR watcher] started  region=({x1},{y1})→({x2},{y2})")

            def _preprocess(raw_img):
                # 2x upscale → greyscale → high contrast → sharpen → threshold
                w, h = raw_img.size
                img = raw_img.resize((w * 2, h * 2), Image.LANCZOS)
                img = img.convert("L")                         # greyscale
                img = ImageEnhance.Contrast(img).enhance(3.0)  # punch contrast
                img = img.filter(ImageFilter.SHARPEN)
                # binarise: pixels above mid-grey → white, else black
                img = img.point(lambda p: 255 if p > 128 else 0, "1")
                return img

            while self._ocr_watch_running and self._craft_running:
                try:
                    with mss.mss() as sct:
                        mon  = {"left": x1, "top": y1,
                    ...
                    # run OCR on both the raw and preprocessed image for best hit rate
                    cfg = "--psm 6 -c tessedit_char_whitelist=abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 !'"
                    text_raw  = pytesseract.image_to_string(raw,             config=cfg).lower()
                    text_proc = pytesseract.image_to_string(_preprocess(raw), config=cfg).lower()
                    combined  = text_raw + " " + text_proc
                    print(f"[OCR watcher] raw={text_raw!r}")
                    print(f"[OCR watcher] proc={text_proc!r}")

                    if any(ph in combined for ph in _BAD_PHRASES):
                        print("[OCR watcher] *** DETECTED ***")
                        self._ocr_watch_result = True
                        try:
                            self.root.after(0, lambda: self._ac_status.set(
                                "⚠ OCR: not enough ingredients!"))
                        except RuntimeError:
                            pass
                        self._craft_fire_webhook(-1, s)
                        break
                except Exception as e:
                    print(f"[OCR watcher] scan error: {e}")
                time.sleep(0.5)
            print("[OCR watcher] stopped")

        def _craft_fire_webhook(self, loop_num, snap=None):
            """Fire the 'out of ingredients' webhook. Safe to call from any thread."""
            try:
                if snap is None:
                    snap = getattr(self, "_craft_snap", {})
                wh_url  = snap.get("wh_url",  "")
                disc_id = snap.get("disc_id", "")
                total   = snap.get("loops",   0)
                if not wh_url:
                    print("[AutoCraft] webhook skipped — no URL")
                    return
                ts = time.strftime("%H:%M:%S")
                loop_txt = f"loop **{loop_num}/{total}**" if loop_num > 0 else "background scan"
                embed = {
                    "title":       f"[{ts}] 🔨 Auto Craft — Out of Ingredients",
                    "color":       0xff6b6b,
                    "description": (
                        f"> ⚠ **Not enough ingredients** detected ({loop_txt}).\n"
                        f"> Auto Craft has been stopped."
                    ),
                    "footer": {"text": FOOTER},
                }
                payload = {"embeds": [embed]}
                if disc_id:
                    payload["content"] = f"<@{disc_id}>"
                print(f"[AutoCraft] firing webhook to {wh_url[:40]}…")
                send_webhook(wh_url, payload)
            except Exception as e:
                print(f"[AutoCraft] webhook error: {e}")

        def _craft_click(self, x, y, delay):
            """Focus Roblox then click at absolute screen coords via SendInput."""
            ...
            # 2. SendInput with absolute coords
            #    Primary monitor: 0..65535 maps to 0..screen_width
            #    Must account for virtual desktop offset if multi-monitor
            try:
                import mss as _mss
                with _mss.mss() as _sct:
                    _vm = _mss.mss().monitors[0]   # virtual desktop bounds
                    vx  = _vm["left"]; vy = _vm["top"]
                    vsw = _vm["width"]; vsh = _vm["height"]
            except Exception:
                vx = vy = 0
                try:
                    vsw, vsh = get_screen_size()
                except Exception:
                    vsw, vsh = 1920, 1080

            # map absolute pixel → SendInput normalised (0-65535 over virtual desktop)
            nx = int((x - vx) * 65535 / vsw)
            ny = int((y - vy) * 65535 / vsh)

            # INPUT structure
            class MOUSEINPUT(ctypes.Structure):
                _fields_ = [("dx","l"),("dy","l"),("mouseData","I"),
                             ("dwFlags","I"),("time","I"),("dwExtraInfo","P")]
                _fields_ = [
                    ("dx",          ctypes.c_long),
                    ("dy",          ctypes.c_long),
            ...
            class INPUT(ctypes.Structure):
                ...
                _anonymous_ = ("_inp",)
                _fields_   = [("type", ctypes.c_ulong), ("_inp", _INP)]
            ...
            # smooth move: get current cursor pos, interpolate to target over ~200ms
            try:
                cur_pt = ctypes.wintypes.POINT()
                ctypes.windll.user32.GetCursorPos(ctypes.byref(cur_pt))
                cur_nx = int((cur_pt.x - vx) * 65535 / vsw)
                cur_ny = int((cur_pt.y - vy) * 65535 / vsh)
            except Exception:
                cur_nx, cur_ny = nx, ny

            STEPS = 18
            STEP_SLEEP = 0.011   # ~200ms total travel
            for step in range(1, STEPS + 1):
                t = step / STEPS
                # ease-in-out cubic
                t_ease = t * t * (3 - 2 * t)
                ix = int(cur_nx + (nx - cur_nx) * t_ease)
                iy = int(cur_ny + (ny - cur_ny) * t_ease)
                mv = _make(MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE, ix, iy)
                ctypes.windll.user32.SendInput(1, ctypes.byref(mv), ctypes.sizeof(INPUT))
                time.sleep(STEP_SLEEP)

            down  = _make(MOUSEEVENTF_LEFTDOWN | MOUSEEVENTF_ABSOLUTE, nx, ny)
            up    = _make(MOUSEEVENTF_LEFTUP   | MOUSEEVENTF_ABSOLUTE, nx, ny)
        ...
        def _craft_loop(self):
            def _st(msg):
                try:
                    self.root.after(0, lambda m=msg: self._ac_status.set(m))
                except RuntimeError:
                    pass

            def _lv(msg):
                try:
                    self.root.after(0, lambda m=msg: self._ac_loop_var.set(m))
                except RuntimeError:
                    pass
            ...
            total      = s["loops"]
            d          = s["delay"]
            skip_tick  = s.get("skip_tick", False)

            x1, y1 = self._ac_cal["add_all_1"]
            xt, yt = self._ac_cal["tick_warn"]
            x2, y2 = self._ac_cal["add_all_2"]
            xc, yc = self._ac_cal["craft_btn"]

            for i in range(1, total + 1):
                if not self._craft_running or self._ocr_watch_result:
                    break

                _lv(f"{i} / {total}")

                try:
                    # Step 1 — Add All (every loop)
                    _st(f"Loop {i}/{total} — Add All")
                    self._craft_click(x1, y1, d)
                    if not self._craft_running or self._ocr_watch_result: break

                    # Steps 2+3 — first loop only, and only if skip_tick is NOT checked
                    if i == 1 and not skip_tick:
                        _st(f"Loop {i}/{total} — Tick warning")
                        self._craft_click(xt, yt, d)
                        if not self._craft_running or self._ocr_watch_result: break

                        _st(f"Loop {i}/{total} — Add All (after tick)")
                        self._craft_click(x2, y2, d)
                        if not self._craft_running or self._ocr_watch_result: break

                    # Step 4 — Craft
                    _st(f"Loop {i}/{total} — Craft")
                    self._craft_click(xc, yc, d)
                    if not self._craft_running: break

                    # wait for craft animation then check
                    time.sleep(max(0.4, d * 0.5))

                    if self._ocr_watch_result:
                        _st(f"⚠ Not enough — stopped at loop {i}")
                        _lv(f"{i} / {total}  ⚠")
                        break

                except Exception as e:
                    _st(f"Error on loop {i}: {e}")
                    print(f"[AutoCraft] loop error: {e}")
                    import traceback; traceback.print_exc()
                    break

            if self._craft_running and not self._ocr_watch_result:
                _st(f"✓ Done — {total} loops complete")
            self._craft_running     = False
            self._ocr_watch_running = False
            try:
                self.root.after(0, lambda: self._ac_start_btn.configure(bg=ACC))
            except RuntimeError:
                pass
        ...
        def _build_leaderboard_tab(self):
            ...
            page_header(root, "Leaderboard", "public grind time — powered by Supabase")
            ...
            btn(bf, "Reset My Time", self._lb_reset_me, BG3).pack(side="right", padx=(0,6))
        ...
        def _lb_reset_me(self):
            uname = self._player_name_var.get() if hasattr(self, "_player_name_var") else ""
            if not uname:
                messagebox.showinfo("Reset", "No username set. Enter your Roblox username first.")
                return
            if not messagebox.askyesno("Reset Time", f"Reset your leaderboard time for '{uname}'?"):
                return
            def _do_reset():
                try:
                    resp = requests.delete(
                        f"{SB_URL}/leaderboard?username=eq.{requests.utils.quote(uname)}",
                        headers={**SB_READ_HDR, "Content-Type": "application/json"}, timeout=8)
                    print(f"[LB] reset {uname} status={resp.status_code}")
                    self._session_accum = 0.0
                    self._last_lb_upload = 0.0
                    self.root.after(0, lambda: self._lb_status_var.set("Your time has been reset."))
                    self.root.after(0, lambda: threading.Thread(target=self._lb_load, daemon=True).start())
                except Exception as e:
        ...
        def _lb_render(self, data):
            ...
            if not data:
                tk.Label(self._lb_inner, text="No data yet. Run the macro and close it to save your time!",
        ...
        def _apply_theme_live(self, bg0, bg1, acc):
            old_bg0 = THEME["bg0"]
            old_bg1 = THEME["bg1"]
            old_bg2 = THEME["bg2"]
            old_bg3 = THEME["bg3"]
            ...
            protected = getattr(self, "_swatch_widgets", set())
        ...
        def _build_ui_tab(self):
            root = tk.Frame(self.content_area, bg=BG1); self._reg("UI", root)
            root.configure(padx=18, pady=16)

            page_header(root, "UI Customisation", "changes apply instantly")

            c1 = card(root); c1.pack(fill="x", pady=(0,10))
            sec(c1, "PRESET THEMES", 0, 1); div(c1, 1, 1)

            PRESETS = [
                ("Default Dark",   "#0a0a0a", "#0f0f0f", "#7c6af7"),
                ("Midnight Blue",  "#050a14", "#0a1428", "#5ce1e6"),
                ("Deep Forest",    "#050e08", "#0b1a0e", "#3dba6e"),
                ("Crimson Night",  "#0e0507", "#1a0a0d", "#ff6b9d"),
                ("Gold Rush",      "#0e0a02", "#1a1205", "#e09a00"),
                ("Pure Black",     "#000000", "#070707", "#7c6af7"),
            ]

            preset_frame = tk.Frame(c1, bg=BG2)
            preset_frame.grid(row=2, column=0, columnspan=1, sticky="ew", padx=12, pady=(4,12))
            for col in range(3):
                preset_frame.columnconfigure(col, weight=1)

            def _apply_preset(bg0, bg1, acc):
                try:
                    if "UITheme" not in self.cfg: self.cfg["UITheme"] = {}
                    self.cfg["UITheme"]["bg0"] = bg0
                    self.cfg["UITheme"]["bg1"] = bg1
                    self.cfg["UITheme"]["accent"] = acc
                    self._save_config()
                    self._apply_theme_live(bg0, bg1, acc)
                except Exception as e:
                    messagebox.showerror("Error", str(e), parent=self.root)

            for i, (name, bg0, bg1, acc) in enumerate(PRESETS):
                _bg0, _bg1, _acc = bg0, bg1, acc
                pf = tk.Frame(preset_frame, bg=bg0, cursor="hand2",
                              highlightthickness=1, highlightbackground="#333333")
                pf.grid(row=i//3, column=i%3, padx=4, pady=4, sticky="ew", ipadx=6, ipady=8)
                sw_row = tk.Frame(pf, bg=bg0); sw_row.pack(pady=(5,3))
                for sc in [bg0, bg1, acc]:
                    tk.Frame(sw_row, bg=sc, width=16, height=16,
                             highlightthickness=1, highlightbackground="#444").pack(side="left", padx=1)
                tk.Label(pf, text=name, bg=bg0, fg="#888899",
                         font=("Segoe UI", 7, "bold")).pack(pady=(0,5))
                if not hasattr(self, "_swatch_widgets"):
                    self._swatch_widgets = set()
                for w in [pf, sw_row] + list(pf.winfo_children()) + list(sw_row.winfo_children()):
                    self._swatch_widgets.add(w)
                for w in [pf] + list(pf.winfo_children()) + [sw_row] + list(sw_row.winfo_children()):
                    try:
                        w.bind("<Button-1>", lambda e, b0=_bg0, b1=_bg1, a=_acc: _apply_preset(b0, b1, a))
                    except Exception:
                        pass

            c3 = card(root); c3.pack(fill="x")
            sec(c3, "ACCENT COLOUR", 0, 1); div(c3, 1, 1)

            acc_frame = tk.Frame(c3, bg=BG2)
            acc_frame.grid(row=2, column=0, columnspan=1, sticky="ew", padx=12, pady=(4, 12))
            ...
            tk.Label(acc_frame, text="Click a colour to apply it instantly:",
                     bg=BG2, fg=TXT2, font=FONT_LABEL).pack(anchor="w", pady=(0, 6))
            acc_row = tk.Frame(acc_frame, bg=BG2); acc_row.pack(anchor="w")
            def _apply_accent(c):
                if "UITheme" not in self.cfg:
                    self.cfg["UITheme"] = {}
                self.cfg["UITheme"]["accent"] = c
                self._save_config()
                self._apply_theme_live(
                    self.cfg.get("UITheme", "bg0", fallback=THEME["bg0"]),
                    self.cfg.get("UITheme", "bg1", fallback=THEME["bg1"]),
                    c
                )

            for hex_col, cname in ACCENTS:
                _col = hex_col
                swatch = tk.Frame(acc_row, bg=hex_col, width=28, height=28,
                                  cursor="hand2", highlightthickness=1, highlightbackground="#333")
                swatch.pack(side="left", padx=3)
                swatch.bind("<Button-1>", lambda e, c=_col: _apply_accent(c))
                if not hasattr(self, "_swatch_widgets"):
                    self._swatch_widgets = set()
                self._swatch_widgets.add(swatch)
                tk.Label(acc_row, text=cname, bg=BG2, fg=TXT3, font=("Segoe UI", 7)).pack(side="left", padx=(0, 8))
                ...
                def _press():
                    ...
                    try:
                        self.root.after(0, lambda: self.ak_status_var.set(
                            f"✓ Clicked Roblox — next in {secs}s"))
                    except RuntimeError:
                        pass
                threading.Thread(target=_press, daemon=True).start()
        ...
        def _stop(self):
            ...
            self._stop_keep_chat()
            if getattr(self, "_keep_chat_var", None):
                try: self._keep_chat_var.set(False)
                except: pass
            if self._timer_job:
        ...
        def _check(self):
            ...
            print(f"[Biome poll] detected={biome!r}  last={self.last_biome!r}  url={bool(url)}")
            if biome is None:
                pass
                ...
                if url:
                    if biome in BASE_BIOMES:
                        if prev and prev not in BASE_BIOMES:
                        ...
                        if s != "Nothing":
                            now = time.time()
                            last_sent = self._last_biome_sent_at.get(biome, 0.0)
                            if now - last_sent < 60:
                                print(f"[Biome] suppressed duplicate webhook for {biome} (sent {int(now - last_sent)}s ago)")
                            else:
                                self._last_biome_sent_at[biome] = now
                                threading.Thread(target=send_webhook,
        ...
        _CHAT_FALLBACK_XY = (96, 62)
        _CHAT_FALLBACK_BBOX = (0, 50, 470, 400)
        ...
        def _ocr_scan_chat_box(self):
            ...
            try:
                ...
                grey = np.array(shot.convert("L"), dtype=np.float32)
                H, W = grey.shape

                panel_w = min(CHAT_W_EST, W)
                row_mean = grey[:, :panel_w].mean(axis=1)
        ...
        def _keep_chat_tick(self):
            if not self._keep_chat_open:
                return
            ...
            def _do_click():
                try:
                    ...
                    xy = getattr(self, "_ocr_chat_xy", None)
                    if not xy:
                        bbox = getattr(self, "_ocr_chat_bbox", None)
                        if bbox:
                            x1, y1, x2, y2 = bbox
                            xy = ((x1 + x2) // 2, (y1 + y2) // 2)
                        else:
                            xy = self._CHAT_FALLBACK_XY
                    ...
                    if PAG_OK:
                        ...
                        sw, sh = get_screen_size()
                        cx, cy = sw // 2, sh // 2
                        pyautogui.moveTo(xy[0], xy[1], duration=0.6)
                        _t2.sleep(0.2)
                        pyautogui.click()
                        print(f"[KeepChat] clicked chat at {xy}")
                        _t2.sleep(0.3)
                        pyautogui.moveTo(cx, cy, duration=0.5)
                        print(f"[KeepChat] moved to centre ({cx}, {cy})")
                        _t2.sleep(0.4)
                        pyautogui.moveTo(xy[0], xy[1], duration=0.5)
                        print(f"[KeepChat] returned to chat at {xy}")
                    else:
                        import ctypes
                        found = []
                        def _enum(h, _):
                            buf = ctypes.create_unicode_buffer(256)
                            ctypes.windll.user32.GetWindowTextW(h, buf, 256)
                            if "Roblox" in buf.value and ctypes.windll.user32.IsWindowVisible(h):
                                found.append(h)
                        WNDENUMPROC = ctypes.WINFUNCTYPE(ctypes.c_bool, ctypes.c_void_p, ctypes.c_void_p)
                        ctypes.windll.user32.EnumWindows(WNDENUMPROC(_enum), None)
                        if found:
                            hwnd = found[0]
                            post = ctypes.windll.user32.PostMessageW
                            VK_SLASH, VK_ESC = 0xBF, 0x1B
                            post(hwnd, 0x0100, VK_SLASH, 0)
                            import time as _t; _t.sleep(0.08)
                            post(hwnd, 0x0101, VK_SLASH, 0)
                            _t.sleep(0.35)
                            post(hwnd, 0x0100, VK_ESC, 0)
                            _t.sleep(0.08)
                            post(hwnd, 0x0101, VK_ESC, 0)
                            print("[KeepChat] fallback: slash+esc sent")
                except Exception as e:
                    print(f"[KeepChat] error: {e}")
                finally:
                    if self._keep_chat_open:
                        self._keep_chat_job = self.root.after(8_000, self._keep_chat_tick)

            threading.Thread(target=_do_click, daemon=True).start()

        def _start_keep_chat(self):
            if self._keep_chat_open:
                return
            self._keep_chat_open = True
            self._keep_chat_tick()

        def _stop_keep_chat(self):
            self._keep_chat_open = False
            if self._keep_chat_job:
                try: self.root.after_cancel(self._keep_chat_job)
                except: pass
                self._keep_chat_job = None
        ...
        def _grab_region(x0, y0, x1, y1):
            ...
            w = max(1, x1 - x0)
            h = max(1, y1 - y0)
            print(f"[Grab] requested region: left={x0} top={y0} w={w} h={h}")
            ...
            try:
                ...
                def _enum(h, _):
                    ...
                    if "Roblox" in t and win32gui.IsWindow(h):
                        hwnd = h
                ...
                if hwnd:
                    ...
                    bmp.CreateCompatibleBitmap(dc, win_w, win_h)
                    mdc.SelectObject(bmp)

                    result = ctypes.windll.user32.PrintWindow(hwnd, mdc.GetSafeHdc(), PW_RENDERFULLCONTENT)

                    if result:
                        raw_data = bmp.GetBitmapBits(True)
                        full_img = Image.frombuffer(
                            "RGB", (win_w, win_h),
                            raw_data, "raw", "BGRX", 0, 1)
                        rel_x  = max(0, min(x0 - left, win_w - 1))
                        rel_y  = max(0, min(y0 - top,  win_h - 1))
                        rel_x2 = min(rel_x + w, win_w)
                        rel_y2 = min(rel_y + h, win_h)
                        img = full_img.crop((rel_x, rel_y, rel_x2, rel_y2))
                        mdc.DeleteDC(); dc.DeleteDC()
                        win32gui.ReleaseDC(hwnd, wdc)
                        win32gui.DeleteObject(bmp.GetHandle())
                        print(f"[Grab] PrintWindow OK: {img.size}")
                        return img
                    else:
                        mdc.DeleteDC(); dc.DeleteDC()
                        win32gui.ReleaseDC(hwnd, wdc)
                        win32gui.DeleteObject(bmp.GetHandle())
                        print("[Grab] PrintWindow returned 0 — falling back")
            except Exception as e:
                print(f"[Grab] PrintWindow failed: {e}")
                traceback.print_exc()
        ...
        def _ocr_read_chat_text(self, save_debug=False, bbox=None):
            ...
            try:
                ...
                best_text = ""
                for threshold in [15, 30, 50, 80]:
                    max_ch = _np.max(arr, axis=2)
                    mask = (max_ch > threshold).astype(_np.uint8) * 255
                    binary = 255 - mask
                ...
                try:
                    ...
                    raw2 = pytesseract.image_to_string(grey, config="--psm 6 --oem 3")
                    raw2 = raw2.strip()
                    if len(raw2) > len(best_text):
                        best_text = raw2
                except Exception:
                    pass
                ...
                print(f"[ChatOCR] read: {repr(best_text[:120])}")
        ...
        def _on_close(self):
            ...
            print(f"[LB] on_close: uname={uname!r} session_accum={self._session_accum:.1f} new_secs={new_secs:.1f}")
            if uname and new_secs > 5:
                t = threading.Thread(target=lb_update, args=(uname, new_secs), daemon=True)
                t.start()
                t.join(timeout=12)
            else:
                print(f"[LB] skipped upload: uname={uname!r} new_secs={new_secs:.1f}")
            url = self.wh_url.get().strip()
            if self.started and url:
